// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot.commands.Shooter;

import edu.wpi.first.wpilibj2.command.Command;
import frc.robot.subsystems.ShooterCrawlSubsystem;
import frc.robot.subsystems.ShooterSubsystem;

public class ShooterCrawl extends Command {
  private ShooterCrawlSubsystem shooterCrawlSubsystem;
  private boolean isCrawl;

  /** Creates a new ShooterCrawl. */
  public ShooterCrawl(ShooterCrawlSubsystem shooterCrawlSubsystem, boolean isCrawl) {
    // Use addRequirements() here to declare subsystem dependencies.
   this.shooterCrawlSubsystem = shooterCrawlSubsystem;
    this.isCrawl = isCrawl;

    addRequirements(shooterCrawlSubsystem);
  }

  // Called when the command is initially scheduled.
  @Override
  public void initialize() {
    if(isCrawl){
      shooterCrawlSubsystem.CrawlFwd();
    }else{
      shooterCrawlSubsystem.CrawlRev();
    }
  }

  // Called every time the scheduler runs while the command is scheduled.
  @Override
  public void execute() {}

  // Called once the command ends or is interrupted.
  @Override
  public void end(boolean interrupted) {
   shooterCrawlSubsystem.CrawlStop();
  }

  // Returns true when the command should end.
  @Override
  public boolean isFinished() {
    return false;
  }
}
